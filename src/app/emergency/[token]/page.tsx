...
export default function EmergencyBoardPage() {
  ...
  if (phase === 'error') return <Centered><div style={{ fontSize: 40 }}>⚠️</div><div style={{ marginTop: 8, fontWeight: 700 }}>{error}</div><div style={{ color: '#64748b', fontSize: 13, marginTop: 6 }}>請向家屬索取新的連結。</div></Centered>;

  // ── Break-glass gate ──────────────────────────────────────────────────────
  if (phase === 'gate') {
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 380, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: 24, boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
          <div style={{ fontSize: 13, color: '#be123c', fontWeight: 800 }}>HealthKeep · 急診保命資料</div>
          <h1 style={{ fontSize: 20, fontWeight: 850, margin: '6px 0 2px', color: '#0f172a' }}>{info?.patient_label}</h1>
          <div style={{ fontSize: 12, color: '#64748b' }}>連結有效至 {info ? new Date(info.expires_at).toLocaleString('zh-TW') : ''}</div>
          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', color: '#9a3412', borderRadius: 10, padding: 10, fontSize: 12, lineHeight: 1.6, margin: '14px 0' }}>
            為保護病人隱私，檢視前請留下您的身分。<strong>每次存取都會記錄</strong>並讓家屬看到。
          </div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#334155', marginBottom: 4 }}>您的姓名 *</label>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="例：王醫師"
            style={inp} />
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#334155', margin: '12px 0 4px' }}>院所 / 科別（選填）</label>
          <input value={org} onChange={e => setOrg(e.target.value)} placeholder="例：台大急診"
            style={inp} />
          {error && <div style={{ color: '#be123c', fontSize: 13, marginTop: 10 }}>{error}</div>}
          <button onClick={open} disabled={opening}
            style={{ width: '100%', marginTop: 16, background: '#be123c', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontWeight: 850, fontSize: 15, cursor: 'pointer' }}>
            {opening ? '載入中…' : '檢視保命資料'}
          </button>
        </div>
      </Centered>
    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9' }}>
      <div style={{ maxWidth: 820, margin: '0 auto', padding: '14px 14px 60px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap', padding: '6px 2px 12px' }}>
          <div>
            <div style={{ fontSize: 12, color: '#be123c', fontWeight: 800 }}>急診保命戰術板</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#0f172a' }}>
              {b.patient.name} <span style={{ fontSize: 14, fontWeight: 600, color: '#475569' }}>· {b.patient.age ? `${b.patient.age}y` : '年齡未記錄'} / {b.patient.sex ?? '—'}</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={copySummary} style={{ background: '#0f172a', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 14px', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>複製 HIS 摘要</button>
            <button onClick={downloadSummary} style={{ background: '#fff', color: '#0f172a', border: '1px solid #cbd5e1', borderRadius: 10, padding: '10px 14px', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>下載 TXT</button>
          </div>
        </div>

        {/* Tier 1 保命紅區 */}
        <Section title="🔴 保命紅區 · Tier 1" tone="#be123c">
          {t1.length === 0 ? <Empty>目前無 Tier 1 紅區資料</Empty> : t1.map(it => <RZRow key={it.id} it={it} />)}
        </Section>

        {/* 現用藥 */}
        <Section title="💊 現用藥" tone="#1d4ed8">
          {b.medications.length === 0 ? <Empty>無使用中藥物紀錄</Empty> : b.medications.map((m, i) => (
            <div key={i} style={row}>
              <div><strong style={{ color: '#0f172a' }}>{m.drug_name}</strong> <span style={{ color: '#475569', fontSize: 13 }}>{[m.dose, m.frequency].filter(Boolean).join(' · ')}</span></div>
        ...
        {/* Tier 2 重要背景 */}
        <Section title="🟡 重要背景 · Tier 2" tone="#a16207">
          {t2.length === 0 ? <Empty>無 Tier 2 資料</Empty> : t2.map(it => <RZRow key={it.id} it={it} />)}
        </Section>

        {/* Problem 清單 */}
        <Section title="🩺 疾病 / 問題清單" tone="#7c3aed">
          {b.problems.length === 0 ? <Empty>無已發布的 Problem</Empty> : b.problems.map((p, i) => (
            <div key={i} style={row}>
              <div><strong style={{ color: '#0f172a' }}>{p.name}</strong>{p.layman ? <span style={{ color: '#64748b', fontSize: 12 }}> · {p.layman}</span> : ''}</div>
        ...
        {/* 生命徵象 */}
        <Section title="📈 最新生命徵象" tone="#0f766e">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 8 }}>
              {b.vitals.map((v, i) => (
                <div key={i} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 10 }}>
                  <div style={{ fontSize: 11, color: '#64748b', fontWeight: 700 }}>{VLABEL[v.type] ?? v.type}</div>
                  <div style={{ fontSize: 17, fontWeight: 850, color: '#0f172a' }}>
                    {v.type === 'blood_pressure' && v.value2 ? `${v.value1}/${v.value2}` : v.value1}<span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}> {v.unit ?? ''}</span>
                  </div>
                  <div style={{ fontSize: 10, color: '#94a3b8' }}>{fmt(v.recorded_at)}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <Section title="原始文件索引" tone="#475569">
          {b.evidence_documents.length === 0 ? <Empty>此急診連結目前沒有可追溯的原始文件索引</Empty> : b.evidence_documents.map((d) => (
            <div key={d.id} style={row}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12, color: '#64748b', fontWeight: 700 }}>{d.doc_type} · {fmt(d.doc_date)}</div>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', wordBreak: 'break-all' }}>{d.file_name}</div>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>{d.status} · {d.linked_to_verified_data ? '已連結整理資料' : '尚未連結整理資料'}</div>
              </div>
        ...
        <Section title="DICOM 檢視連結" tone="#6d28d9">
          {b.dicom_links.length === 0 ? <Empty>此急診連結目前沒有已開放的 DICOM viewer 連結</Empty> : b.dicom_links.map((d) => (
            <div key={d.share_id} style={row}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12, color: '#64748b', fontWeight: 700 }}>{d.modality || '影像'} · {d.study_date || '日期未記錄'}</div>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a' }}>{d.study_description || d.series_description || 'DICOM Viewer'}</div>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>viewer 有效至 {d.expires_at ? new Date(d.expires_at).toLocaleString('zh-TW') : '未記錄'}</div>
              </div>
        ...
        <div style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.7, marginTop: 14 }}>{b.disclaimer}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={row}>
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 800, color: '#0f172a', lineHeight: 1.35 }}>{it.value}</div>
        <div style={{ fontSize: 10, color: '#94a3b8' }}>{it.source} · {fmt(it.last_reviewed_at)}</div>
      </div>
...
function Section({ title, tone, children }: { title: string; tone: string; children: React.ReactNode }) {
  return (
    <section style={{ background: '#fff', border: '1px solid #e2e8f0', borderLeft: `4px solid ${tone}`, borderRadius: 12, padding: '12px 14px', marginBottom: 12 }}>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <div style={{ color: '#94a3b8', fontSize: 13, padding: '4px 0' }}>{children}</div>;
}
...
const inp: React.CSSProperties = { width: '100%', border: '1px solid #cbd5e1', borderRadius: 10, padding: '10px 12px', fontSize: 15 };
const row: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '1px solid #f1f5f9' };
const linkBtn: React.CSSProperties = { border: '1px solid #cbd5e1', background: '#fff', color: '#0f172a', borderRadius: 8, padding: '7px 10px', fontWeight: 800, fontSize: 12, textDecoration: 'none', flexShrink: 0 };
